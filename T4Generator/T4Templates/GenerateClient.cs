...
namespace Clients
{
    ...
    { 
        ...
        { 
            ...
            var request = CreateRequest(userContext, HttpMethod.Post, url);
            request.Content = new ObjectContent<SampleClassLibrary.SampleInputContract>(contract, _formatters[0]);
            SendRequest(request);
        }

        public void UpdatePredefinedViews(UserContextContract userContext, bool isTsComplete)
        { 
            var url = $"views/update-predefined/{isTsComplete}";
            var request = CreateRequest(userContext, HttpMethod.Post, url);
    ...
    {
        IEnumerable<System.DateTime> PostSomething(UserContextContract userContext, System.DateTime date);
        decimal GetCrossCourse(UserContextContract userContext, int fromCurrencyId, int toCurrencyId, System.DateTime tradeDate);
        IEnumerable<int> Get(UserContextContract userContext);
        void Delete(UserContextContract userContext, int id);
        void RemoveDsiColumnFromUserViews(UserContextContract userContext);
        List<SampleClassLibrary.SampleOutputContract> GetAllAvailableColumns(UserContextContract userContext, SampleClassLibrary.SampleEnum enumParameter);
        void SaveColumnSort(UserContextContract userContext, SampleClassLibrary.SampleInputContract contract);
        void UpdatePredefinedViews(UserContextContract userContext, bool isTsComplete);
    }
}
